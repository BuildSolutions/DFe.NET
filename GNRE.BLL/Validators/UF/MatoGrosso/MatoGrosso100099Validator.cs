...
{
    public class MatoGrosso100099Validator : AbstractValidator<Configuracao.Entidades.GNRE>
    {
        public MatoGrosso100099Validator()
        {
            ...
            RuleFor(uf => uf.camposExtras).NotNull().WithMessage($"Campos Extras: {ETipoCampoExtraMT.ChaveAcessoNFe.Descricao()} não informado!")
                .DependentRules(() => RuleFor(uf => uf.camposExtras[0].Valor).Length(44).WithMessage($"Campos Extras: {ETipoCampoExtraMT.ChaveAcessoNFe.Descricao()} inválida"));
        }
    }
}
