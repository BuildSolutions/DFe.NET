...
namespace NFe.BLL.Validators
{
    public class NotaFiscalValidator : AbstractValidator<NotaFiscal>
    {
        public NotaFiscalValidator(DFe.Classes.Flags.ModeloDocumento modeloDocumento)
        {
            ...
            RuleFor(nfe => nfe.DataEmissao.Date).GreaterThanOrEqualTo(DateTime.Now.Date).When(nfe => nfe.DeveValidarDataEmissaoAtrasada).WithMessage(_ => $"Data de Emissão muito atrasada (Fora de Prazo).");
            ...
            RuleFor(nfe => nfe.Total.TotalizadorICMS.NFeValorTotal).Equal(parc => parc.Duplicatas.Sum(dup => dup.Valor)).When(nfe => nfe.DeveValidarValorTotalDaNfeEDuplicatas && nfe.Duplicatas?.Count > 0).WithMessage("Valor Total da NF-e não corresponde com o Somatório Total das duplicatas.");
