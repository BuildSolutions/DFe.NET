...
{
    public class Pernambuco100102Validator : AbstractValidator<Configuracao.Entidades.GNRE>
    {
        public Pernambuco100102Validator()
        {
            ...
            RuleFor(uf => uf.camposExtras).NotNull().WithMessage($"Campos Extras: {ETipoCampoExtraPE.ChaveAcessoNFeCTe.Descricao()} não informado!")
                .DependentRules(() => RuleFor(uf => uf.camposExtras[0].Valor).Length(44).WithMessage($"Campos Extras: {ETipoCampoExtraPE.ChaveAcessoNFeCTe.Descricao()} inválida"));
        }
    }
}
