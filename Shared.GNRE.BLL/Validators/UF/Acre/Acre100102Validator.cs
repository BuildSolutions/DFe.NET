...
using GNRE.BLL.Configuracao.Entidades.GNREUF.Acre;

namespace GNRE.BLL.Validators.UF.Acre
{
    public class Acre100102Validator : AbstractValidator<Configuracao.Entidades.GNRE>
    {
        public Acre100102Validator()
        {
            ...
            RuleFor(uf => uf.camposExtras).NotNull().WithMessage($"Campos Extras: {ETipoCampoExtraAC.ChaveAcessoNFe.Descricao()} não informado!")
                .DependentRules(() => RuleFor(uf => uf.camposExtras[0].Valor).Length(44).WithMessage($"Campos Extras: {ETipoCampoExtraAC.ChaveAcessoNFe.Descricao()} inválida"));
        }
    }
}
